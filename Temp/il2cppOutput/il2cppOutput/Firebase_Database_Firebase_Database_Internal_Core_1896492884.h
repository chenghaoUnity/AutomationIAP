#pragma once

#include "il2cpp-config.h"

#ifndef _MSC_VER
# include <alloca.h>
#else
# include <malloc.h>
#endif

#include <stdint.h>

// Firebase.Database.Internal.Core.Repo
struct Repo_t1244308462;
// System.Collections.Generic.IList`1<Firebase.Database.Internal.Core.View.Event>
struct IList_1_t1273747003;

#include "mscorlib_System_Object2689449295.h"

#ifdef __clang__
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Winvalid-offsetof"
#pragma clang diagnostic ignored "-Wunused-variable"
#endif

// Firebase.Database.Internal.Core.Repo/SparseSnapshotTreeVisitor559
struct  SparseSnapshotTreeVisitor559_t1896492884  : public Il2CppObject
{
public:
	// Firebase.Database.Internal.Core.Repo Firebase.Database.Internal.Core.Repo/SparseSnapshotTreeVisitor559::_enclosing
	Repo_t1244308462 * ____enclosing_0;
	// System.Collections.Generic.IList`1<Firebase.Database.Internal.Core.View.Event> Firebase.Database.Internal.Core.Repo/SparseSnapshotTreeVisitor559::_events
	Il2CppObject* ____events_1;

public:
	inline static int32_t get_offset_of__enclosing_0() { return static_cast<int32_t>(offsetof(SparseSnapshotTreeVisitor559_t1896492884, ____enclosing_0)); }
	inline Repo_t1244308462 * get__enclosing_0() const { return ____enclosing_0; }
	inline Repo_t1244308462 ** get_address_of__enclosing_0() { return &____enclosing_0; }
	inline void set__enclosing_0(Repo_t1244308462 * value)
	{
		____enclosing_0 = value;
		Il2CppCodeGenWriteBarrier(&____enclosing_0, value);
	}

	inline static int32_t get_offset_of__events_1() { return static_cast<int32_t>(offsetof(SparseSnapshotTreeVisitor559_t1896492884, ____events_1)); }
	inline Il2CppObject* get__events_1() const { return ____events_1; }
	inline Il2CppObject** get_address_of__events_1() { return &____events_1; }
	inline void set__events_1(Il2CppObject* value)
	{
		____events_1 = value;
		Il2CppCodeGenWriteBarrier(&____events_1, value);
	}
};

#ifdef __clang__
#pragma clang diagnostic pop
#endif
